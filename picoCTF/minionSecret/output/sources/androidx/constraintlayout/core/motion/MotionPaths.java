...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float dpos_x = (float) (((double) drx) + (d2 * Math.sin(angle)) + (((double) radius) * Math.cos(angle) * ((double) dangle)));
            ...
            if (!Float.isNaN(path_rotate4)) {
                ...
                float rot2 = (float) (((double) 0.0f) + ((double) path_rotate4) + Math.toDegrees(Math.atan2(dy, dx)));
